...
// of Readers are encouraged to experiment on other algorithms (especially for
// better efficiency!)
#include <cctype>
#include <cstdio>
#include <cstring>
...
int main() {
    ...
    while (scanf("%d%d%d\n", &w, &h, &n) == 3 && n) {
        ...
        int cnt, x[maxn], y[maxn],
            id[maxs][maxs]; // cnt is the number of empty cells
        cnt = 0;
        for (int i = 0; i < h; i++)
            for (int j = 0; j < w; j++)
                if (maze[i][j] != '#') {
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            for (int dir = 0; dir < 5; dir++) {
                ...
                // "Outermost cells of a map are walls" means we don't need to
                // check out-of-bound
                if (maze[nx][ny] != '#')
                    G[i][deg[i]++] = id[nx][ny];
            }
